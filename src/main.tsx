  ...
  function renderTimeView(){
    ...
    return <div className="calendar-week">
      <div ref={calendarShellRef} className="calendar-grid-shell" style={{gridTemplateRows:`50px ${frameMinutes*CAL_PX_PER_MIN}px`,minHeight:50+frameMinutes*CAL_PX_PER_MIN}}>
        ...
        {visibleDays.map(day=>{const sealed=isDaySealed(state,day); const past=isPast(state.weekStartIso,day); const today=isToday(state.weekStartIso,day); return <div key={day} className={`calendar-day-head ${today?"today":""} ${past?"past":""} ${sealed?"sealed":""}`} onClick={()=>setState(s=>({...s,selectedDay:day}))} title={dateTitle(state.weekStartIso,day)}>{sealed&&<div className="seal-trail mini" aria-hidden="true"><span/><span/></div>}<strong>{day.toUpperCase()}</strong><small>{dateShort(state.weekStartIso,day)}</small>{sealed&&<span>Locked</span>}</div>})}
