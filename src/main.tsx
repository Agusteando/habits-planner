...
type Category = "Body" | "Food" | "Dopamine" | "System";
...
const CATEGORY_OPTIONS: Category[] = ["Body","Food","Dopamine","System"];
const KIND_OPTIONS: Kind[] = ["discipline","reward","recovery","review","pause"];
const SYSTEM_TASK_IDS = new Set(["recovery","review","pause"]);
const PINNED_SEED_TASK_IDS = new Set(["game","doomscroll"]);
const CATEGORY_ACCENTS: Record<Category, Accent> = {Body:"mint",Food:"gold",Dopamine:"violet",System:"silver"};
const LEGACY_CATEGORY_MAP: Record<string, Category> = {Body:"Body",Food:"Food",Dopa:"Dopamine",Dopamine:"Dopamine",Focus:"System",Recovery:"System",System:"System"};
...
const FUTURE_BLOCK_OFFSET_MINUTES = 120;
...
function tiers(labels:[string,string,string], minutes:[number,number,number], xp:[number,number,number], tension:[number,number,number], relief:[number,number,number]=[0,0,0]): Tier[] {
  return labels.map((label,i)=>({level:(i+1) as 1|2|3,label,minutes:minutes[i],xp:xp[i],tension:tension[i],relief:relief[i]}));
}
function actionTask(id:string,title:string,kind:Kind,category:Category,accent:Accent,tokenCost:number,tokenEarn:number,labels:[string,string,string],minutes:[number,number,number],xp:[number,number,number],tension:[number,number,number],relief:[number,number,number]=[0,0,0]): Task {
  return normalizeTask({id,title,icon:"sprite",kind,category,tokenCost,tokenEarn,accent,tiers:tiers(labels,minutes,xp,tension,relief)});
}

const tasksSeed: Record<string, Task> = {
  pushups: actionTask("pushups","Push-ups","discipline","Body","cyan",0,1,["10 min form","Push-ups 2x8","Push-ups 3x10"],[10,14,18],[18,32,48],[10,18,26]),
  walk: actionTask("walk","Walk","discipline","Body","mint",0,1,["5 min outside","12 min walk","20 min walk"],[5,12,20],[16,26,36],[5,10,16],[2,4,8]),
  avena_walk: actionTask("avena_walk","Avena Walk","discipline","Body","mint",0,1,["Potty loop","12 min dog walk","20 min dog walk"],[6,12,20],[16,28,40],[6,12,18],[2,4,7]),
  plank: actionTask("plank","Plank","discipline","Body","cyan",0,1,["20 sec hold","45 sec hold","90 sec total"],[3,5,8],[14,24,38],[6,12,18]),
  isometrics: actionTask("isometrics","Isometrics","discipline","Body","cyan",0,1,["One hold","Three holds","Full circuit"],[5,10,18],[16,30,46],[8,16,26]),
  rope_jumping: actionTask("rope_jumping","Rope Jumping","discipline","Body","mint",0,1,["3 min rope","8 min intervals","15 min rope"],[3,8,15],[18,34,54],[10,20,32]),
  bike_cardio: actionTask("bike_cardio","Bike Cardio","discipline","Body","mint",0,1,["10 min easy","20 min ride","35 min cardio"],[10,20,35],[20,38,62],[10,22,38],[2,4,8]),
  public_squats: actionTask("public_squats","Public Squats","discipline","Body","cyan",0,1,["10 squats","25 squats","50 squats"],[3,5,8],[14,26,42],[8,16,26]),
  weight_lifting: actionTask("weight_lifting","Weight Lifting","discipline","Body","cyan",0,1,["One lift block","Main lifts","Full session"],[15,35,60],[30,65,95],[24,48,72]),
  chest_day: actionTask("chest_day","Chest Day","discipline","Body","cyan",0,1,["Chest activation","Chest work sets","Chest session"],[12,30,50],[24,56,86],[18,42,66]),
  back_day: actionTask("back_day","Back Day","discipline","Body","cyan",0,1,["Back activation","Rows or pulls","Back session"],[12,30,50],[24,56,86],[18,42,66]),
  shoulder_day: actionTask("shoulder_day","Shoulder Day","discipline","Body","cyan",0,1,["Mobility + press","Shoulder work sets","Shoulder session"],[12,30,50],[24,56,86],[18,42,66]),
  bicep_day: actionTask("bicep_day","Bicep Day","discipline","Body","cyan",0,1,["Curl primer","Bicep work sets","Bicep session"],[10,25,40],[22,48,72],[16,34,54]),
  tricep_day: actionTask("tricep_day","Tricep Day","discipline","Body","cyan",0,1,["Tricep primer","Tricep work sets","Tricep session"],[10,25,40],[22,48,72],[16,34,54]),
  forearm_day: actionTask("forearm_day","Forearm Day","discipline","Body","cyan",0,1,["Grip primer","Forearm work sets","Forearm session"],[8,20,35],[18,40,62],[14,30,48]),
  core_day: actionTask("core_day","Core Day","discipline","Body","cyan",0,1,["Core primer","Core circuit","Core session"],[8,20,35],[18,42,66],[14,32,52]),
  leg_day: actionTask("leg_day","Leg Day","discipline","Body","cyan",0,1,["Leg activation","Leg work sets","Leg session"],[12,35,60],[26,64,98],[22,50,78]),
  quad_day: actionTask("quad_day","Quad Day","discipline","Body","cyan",0,1,["Quad primer","Quad work sets","Quad session"],[10,25,45],[22,50,78],[18,38,60]),
  hamstring_day: actionTask("hamstring_day","Hamstring Day","discipline","Body","cyan",0,1,["Hinge primer","Hamstring work sets","Hamstring session"],[10,25,45],[22,50,78],[18,38,60]),
  glute_day: actionTask("glute_day","Glute Day","discipline","Body","cyan",0,1,["Glute activation","Glute work sets","Glute session"],[10,25,45],[22,50,78],[18,38,60]),
  calf_day: actionTask("calf_day","Calf Day","discipline","Body","cyan",0,1,["Calf primer","Calf work sets","Calf session"],[8,18,30],[18,36,56],[12,26,42]),
  cold_shower: actionTask("cold_shower","Cold Shower","discipline","Body","blue",0,1,["30 sec cold","90 sec cold","3 min cold"],[4,6,8],[16,28,42],[8,15,24],[2,4,6]),
  early_bedtime: actionTask("early_bedtime","Early Bedtime","discipline","Body","blue",0,1,["Screen off","In bed early","Lights out early"],[10,20,30],[18,34,52],[8,18,28],[8,14,22]),
  early_bird: actionTask("early_bird","Early Bird","discipline","Body","gold",0,1,["Up on alarm","Out of bed","Morning launch"],[5,10,20],[18,32,50],[8,16,26],[4,8,12]),
  fasting: actionTask("fasting","Fasting","discipline","Food","gold",0,1,["Delay first bite","12 hour fast","16 hour fast"],[5,10,15],[18,36,58],[10,22,36],[2,6,10]),
  tuna: actionTask("tuna","Tuna Bowl","discipline","Food","gold",0,1,["Protein snack","Tuna bowl","Cook full meal"],[5,20,35],[15,50,70],[6,16,24],[2,3,5]),
  boiled_egg: actionTask("boiled_egg","Boiled Egg","discipline","Food","gold",0,1,["Boil one egg","Protein pair","Meal prep eggs"],[8,12,20],[14,26,40],[5,10,16],[2,4,6]),
  drink_water: actionTask("drink_water","Drink Water","discipline","Food","gold",0,1,["One full glass","Bottle refill","Hydration check"],[2,4,6],[10,18,28],[3,7,12],[4,7,10]),
  bring_lunch: actionTask("bring_lunch","Bring Lunch","discipline","Food","gold",0,1,["Pack simple lunch","Bring planned lunch","Prep tomorrow lunch"],[10,18,25],[24,42,58],[12,22,32],[4,8,12]),
  portion_control: actionTask("portion_control","Portion Control","discipline","Food","gold",0,1,["Plate check","Stop at one serving","Clean portion day"],[3,5,8],[16,30,46],[8,16,26],[3,6,10]),
  skip_oxxo: actionTask("skip_oxxo","Skip OXXO","discipline","Food","gold",0,1,["Pass one stop","Use backup snack","No OXXO day"],[2,5,8],[16,28,42],[8,16,26],[3,6,10]),
  tidy_room: actionTask("tidy_room","Tidy Room","discipline","System","silver",0,1,["5 min reset","Clear surfaces","Full room reset"],[5,15,30],[16,32,52],[6,14,24],[6,12,20]),
  game: actionTask("game","Videogames","reward","Dopamine","violet",2,0,["10 min game","20 min game","30 min hard stop"],[10,20,30],[0,0,0],[0,0,0],[10,18,26]),
  doomscroll: actionTask("doomscroll","Doomscrolling","reward","Dopamine","violet",1,0,["5 min check","10 min cap","15 min hard cap"],[5,10,15],[0,0,0],[0,0,0],[5,9,12]),
  pizza: actionTask("pizza","Pizza Night","reward","Dopamine","rose",3,0,["Small treat","Pizza meal","Pizza + movie"],[15,35,60],[0,0,0],[0,0,0],[16,28,42]),
  recovery: actionTask("recovery","Recovery","recovery","System","amber",0,0,["Water + breathe","Room reset","Full reset ritual"],[15,20,30],[12,18,28],[0,2,4],[12,18,28]),
  review: actionTask("review","Review Window","review","System","silver",0,0,["12 hour review window","Strategic weekly review","Reseal checkpoint"],[720,720,720],[0,0,0],[0,0,0],[8,10,12]),
  pause: actionTask("pause","Pause Day","pause","System","silver",0,0,["Pause remaining day","Protect evening","Full stop"],[0,0,0],[0,0,0],[0,0,0],[20,30,40])
};
...
function weekLabel(iso:string){const s=new Date(iso); const e=addDays(s,6); const month=(d:Date)=>new Intl.DateTimeFormat(undefined,{month:"short"}).format(d); return s.getFullYear()===e.getFullYear()&&s.getMonth()===e.getMonth()?`${month(s)} ${s.getDate()}-${e.getDate()}, ${e.getFullYear()}`:`${month(s)} ${s.getDate()}-${month(e)} ${e.getDate()}, ${e.getFullYear()}`;}
...
function countdown(target?:string){if(!target)return""; const ms=new Date(target).getTime()-Date.now(); if(ms<=0)return"Ready"; const totalSeconds=Math.ceil(ms/1000); const totalMinutes=Math.ceil(ms/60000); const h=Math.floor(totalMinutes/60), min=totalMinutes%60, sec=totalSeconds%60; if(h>=24)return`${Math.floor(h/24)}d ${h%24}h`; return h?`${h}h ${min}m`:totalMinutes>1?`${Math.max(0,totalMinutes-1)}m ${sec}s`:`${sec}s`; }
...
function normalizeCategory(value:any, kind?:Kind):Category {
  if(typeof value==="string"&&LEGACY_CATEGORY_MAP[value]) return LEGACY_CATEGORY_MAP[value];
  if(kind==="reward") return "Dopamine";
  if(kind==="recovery"||kind==="review"||kind==="pause") return "System";
  return "Body";
}
function isDopamineTask(t:Pick<Task,"kind"|"category">){return t.kind==="reward"||t.category==="Dopamine";}
function normalizeTask(t:Task):Task {
  ...
  return {
    ...
    tokenCost:Math.max(0,Math.floor(t.tokenCost||0)),
    tiers:t.tiers.map(tier=>({...tier,xp:dopamine?0:Math.max(0,Math.floor(tier.xp||0))}))
  };
}
function sanitizeTasks(input:Record<string,Task>){const tasks:Record<string,Task>={}; Object.values(input).forEach(t=>{if(t?.id)tasks[t.id]=normalizeTask(t);}); return tasks;}
function mergeTaskBank(userTasks:any){const tasks=sanitizeTasks({...tasksSeed,...(userTasks||{})}); PINNED_SEED_TASK_IDS.forEach(id=>{if(tasksSeed[id])tasks[id]=tasksSeed[id];}); return tasks;}
function effectiveTierXp(t:Task,tier:Tier){return isDopamineTask(t)?0:tier.xp;}
function tokenDeltaForTask(t:Task){return (isDopamineTask(t)?0:t.tokenEarn)-((t.kind==="reward"||t.category==="Dopamine")?t.tokenCost:0);}
function blockTiming(t:Task,b:Block,week:Week,weekStartIso:string,reviewLive:boolean):{label:string;detail:string;progress:number;tone:"review"|"pause"|"recovery"|"cooldown"}|null{
  if(t.kind==="review"){
    const start=reviewWindowStart(weekStartIso,b);
    const end=new Date(start.getTime()+REVIEW_WINDOW);
    if(reviewLive&&week.reviewOpenUntil)return{label:`Review ${countdown(week.reviewOpenUntil)}`,detail:`Open until ${formatExactDateTime(week.reviewOpenUntil)}`,progress:progressBetween(start.toISOString(),week.reviewOpenUntil),tone:"review"};
    return{label:"Review window",detail:`${start.toLocaleString([], {weekday:"short",hour:"numeric",minute:"2-digit"})}–${end.toLocaleTimeString([], {hour:"numeric",minute:"2-digit"})}`,progress:0,tone:"review"};
  }
...
function xpUnlockedForDay(s:State, day:DayKey){return !!s.week.daySeals?.[day]||s.week.mode==="sealed";}
...
function isRecoveryBlock(s:State,b:Block){return s.tasks[b.taskId]?.kind==="recovery";}
function recoveryBlockForDay(s:State,day:DayKey){return s.blocks.find(b=>b.day===day&&isRecoveryBlock(s,b)&&(b.status==="planned"||b.status==="recoveryDue"));}
function isPauseDay(s:State,day:DayKey){return s.blocks.some(b=>b.day===day&&s.tasks[b.taskId]?.kind==="pause"&&(b.status==="planned"||b.status==="done"||b.status==="paused"));}
function isReviewBlock(s:State,b:Block){return s.tasks[b.taskId]?.kind==="review";}
function dayKeyForDate(weekStartIso:string,d=new Date()){return DAYS.find(day=>sameDate(dayDate(weekStartIso,day),d));}
function reviewStartMinutes(b:Block){if(typeof b.timeStart==="number")return clamp(b.timeStart,CAL_START,CAL_END-MIN_EVENT_MINUTES); if(b.timeBand)return bandRange(b.timeBand).start; return REVIEW_DEFAULT_START;}
function reviewWindowStart(weekStartIso:string,b:Block){const start=dayDate(weekStartIso,b.day); const mins=reviewStartMinutes(b); start.setHours(Math.floor(mins/60),mins%60,0,0); return start;}
function reviewWindowEnd(weekStartIso:string,b:Block){return new Date(reviewWindowStart(weekStartIso,b).getTime()+REVIEW_WINDOW);}
function reviewWindowForBlock(s:State,b:Block){const start=reviewWindowStart(s.weekStartIso,b); const end=reviewWindowEnd(s.weekStartIso,b); return{block:b,start,end};}
function reviewEligibleForWeek(s:State,b:Block){if(!isReviewBlock(s,b))return false; if(b.status==="planned")return true; if(b.status==="done"&&(!b.reviewOpenedAt||!sameDate(new Date(b.reviewOpenedAt),dayDate(s.weekStartIso,b.day))))return true; return false;}
function activePlannedReviewWindow(s:State,now=new Date()){if(!hasAnySealed(s))return null; const today=dayKeyForDate(s.weekStartIso,now); if(!today)return null; return dayBlocks(s.blocks,today).map(b=>reviewWindowForBlock(s,b)).find(w=>reviewEligibleForWeek(s,w.block)&&now>=w.start&&now<w.end)||null;}
function currentReviewEnd(s:State){return s.week.reviewOpenUntil?new Date(s.week.reviewOpenUntil):null;}
function reviewWindowIsOpen(s:State,now=new Date()){const end=currentReviewEnd(s); return s.week.mode==="reviewOpen"&&!!end&&now<end;}
function nextReviewWindow(s:State,now=new Date()){const windows=s.blocks.filter(b=>reviewEligibleForWeek(s,b)).map(b=>reviewWindowForBlock(s,b)).filter(w=>w.end>now).sort((a,b)=>a.start.getTime()-b.start.getTime()); return windows[0]||null;}
function reviewNotice(s:State,now=new Date()){if(reviewWindowIsOpen(s,now)){const end=currentReviewEnd(s)!; return{active:true,key:`review-notice:${s.weekStartIso}:open:${s.week.reviewOpenUntil||""}`,title:"Review window open",detail:`All sealed days are editable until ${end.toLocaleTimeString([], {hour:"numeric",minute:"2-digit"})}. Reseal when done.`};} const next=nextReviewWindow(s,now); if(next)return{active:false,key:`review-notice:${s.weekStartIso}:next:${next.block.id}:${next.block.createdAt||""}:${next.start.toISOString()}`,title:"Next review window",detail:`${next.start.toLocaleString([], {weekday:"long",hour:"numeric",minute:"2-digit"})}–${next.end.toLocaleTimeString([], {hour:"numeric",minute:"2-digit"})}`}; return null;}
function noticeCookieName(key:string){return `hpr_${btoa(unescape(encodeURIComponent(key))).replace(/=+$/g,"").replace(/[^A-Za-z0-9_-]/g,"").slice(0,80)}`;}
function readNoticeDismissed(key?:string){if(!key)return false; try{if(localStorage.getItem(key)==="1")return true; const name=noticeCookieName(key)+"="; return document.cookie.split("; ").some(x=>x.startsWith(name));}catch{return false;}}
function dismissNotice(key:string){try{localStorage.setItem(key,"1"); document.cookie=`${noticeCookieName(key)}=1; Max-Age=1209600; Path=/; SameSite=Lax`;}catch{}}
function syncReviewState(s:State,now=new Date()){const openEnd=currentReviewEnd(s); if(s.week.mode==="reviewOpen"&&(!openEnd||now>=openEnd)){const sealed=hasAnySealed(s); return{...s,week:{...s.week,mode:(sealed?"sealed":"draft") as Mode,reviewOpenUntil:undefined,emergencyReviewRequestedAt:undefined,emergencyReviewUnlockAt:undefined},toast:sealed?"Review window closed. Plan resealed.":"Review window closed."};} if(s.week.mode!=="reviewOpen"){const due=activePlannedReviewWindow(s,now); if(due){return{...s,week:{...s.week,mode:"reviewOpen" as Mode,reviewOpenUntil:due.end.toISOString(),emergencyReviewRequestedAt:undefined,emergencyReviewUnlockAt:undefined},blocks:s.blocks.map(b=>b.id===due.block.id?{...b,status:"done" as Status,reviewOpenedAt:now.toISOString()}:b),toast:"Review window open. Sealed days are editable for 12 hours."};}} return s;}
function visibleBands(filter:TimeFilter){return TIME_BANDS.filter(b=>filter==="both"||b.phase===filter);}
function calendarFrame(filter:TimeFilter){const bands=visibleBands(filter); const first=bands[0]||TIME_BANDS[0]; const last=bands[bands.length-1]||TIME_BANDS[TIME_BANDS.length-1]; const end=filter==="morning"?bandRange(last.id).end:CAL_END; return {start:bandRange(first.id).start,end};}
...
function snapForward(n:number,step=15){return Math.ceil(n/step)*step;}
...
function defaultFutureTimeStart(weekStartIso:string,day:DayKey,duration=MIN_EVENT_MINUTES){if(!isToday(weekStartIso,day))return undefined; const now=new Date(); const desired=snapForward(now.getHours()*60+now.getMinutes()+FUTURE_BLOCK_OFFSET_MINUTES); const latest=Math.max(CAL_START,CAL_END-Math.max(MIN_EVENT_MINUTES,duration)); return clamp(desired,CAL_START,latest);}
function defaultTimeStartForNewBlock(weekStartIso:string,day:DayKey,t:Task,target:DropTarget){if(t.kind==="review")return typeof target.timeStart==="number"?target.timeStart:REVIEW_DEFAULT_START; if(typeof target.timeStart==="number"||target.timeBand||target.beforeBlockId)return target.timeStart; const tier=t.tiers.find(x=>x.level===(t.kind==="reward"?2:1))||t.tiers[0]; return defaultFutureTimeStart(weekStartIso,day,Math.max(MIN_EVENT_MINUTES,tier.minutes||30));}
...
function shouldAutoCompleteBlock(weekStartIso:string,b:Block,index:number,t:Task){ if(t.kind==="review"||t.kind==="pause") return false; if(b.status!=="planned") return false; if(!isToday(weekStartIso,b.day)) return false; const {end}=eventTimeMeta(b,index,getTier(t,b.tier)); const now=new Date(); const mins=now.getHours()*60+now.getMinutes(); return mins>=end; }
...
function calendarDuration(b:Block,tier:Tier){if(b.taskId==="review")return REVIEW_WINDOW_MINUTES; return clamp(b.timeMinutes??Math.max(MIN_EVENT_MINUTES,tier.minutes||30),MIN_EVENT_MINUTES,240);}
function ensureReviewBlock(s:State, day:DayKey, unlockAt?:string){return s.blocks;}
...
function App(){
  const [state,setState]=useState<State>(loadState); const [drag,setDrag]=useState<Drag|null>(null); const [expanded,setExpanded]=useState<string|null>(null); const [popover,setPopover]=useState<{blockId:string;top:number;left:number;width:number}|null>(null); const [menu,setMenu]=useState(false); const [resetConfirm,setResetConfirm]=useState(false); const [bankMenu,setBankMenu]=useState(false); const [showAdd,setShowAdd]=useState(false); const [query,setQuery]=useState(""); const [chip,setChip]=useState<"All"|Category>("All"); const [newTitle,setNewTitle]=useState(""); const [newKind,setNewKind]=useState<Kind>("discipline"); const [editingTaskId,setEditingTaskId]=useState<string|null>(null); const [tick,setTick]=useState(0); const [bursts,setBursts]=useState<ParticleBurst[]>([]); const [xpFloats,setXpFloats]=useState<XpFloat[]>([]); const [beamGeom,setBeamGeom]=useState<{top:number;left:number;width:number;hasTarget:boolean}|null>(null); const [hiddenNoticeKey,setHiddenNoticeKey]=useState<string|null>(null); const planImport=useRef<HTMLInputElement|null>(null); const bankImport=useRef<HTMLInputElement|null>(null); const calendarShellRef=useRef<HTMLDivElement|null>(null); const soundRef=useRef<ReturnType<typeof createSoundEngine>|null>(null);
  ...
  useEffect(()=>{setState(s=>syncReviewState(s));},[tick]);
    ...
    for(const day of DAYS){
      ...
      blocks.forEach((b,index)=>{
        ...
        if(shouldAutoCompleteBlock(state.weekStartIso,b,index,t)){
          ...
          pending.push({id:b.id,xp:xpUnlockedForDay(state,b.day)?effectiveTierXp(t,tier):0,tokens:tokenDeltaForTask(t),recovery:t.kind==="recovery"});
        }
      });
    }
  ...
  function addTemplate(taskId:string,target:DropTarget){playSfx(!canEditDay(state,target.day)||!state.tasks[taskId]?"error":"drop"); patch(s=>{if(!canEditDay(s,target.day))return{...s,toast:"That day is sealed."}; const t=s.tasks[taskId]; if(!t)return{...s,toast:"Action not found."}; if(t.kind==="review"&&isToday(s.weekStartIso,target.day))return{...s,toast:"Review blocks must be planned before the review day."}; const timeStart=defaultTimeStartForNewBlock(s.weekStartIso,target.day,t,target); return{...s,selectedDay:target.day,blocks:insertBlock(s.blocks,makeBlock(t,target.day,nextOrder(s.blocks,target.day),undefined,target.timeBand,timeStart),target)};});}
  function moveBlock(blockId:string,target:DropTarget){const b=state.blocks.find(x=>x.id===blockId); playSfx(!b||!canEditDay(state,b.day)||!canEditDay(state,target.day)?"error":"drop"); patch(s=>{const b=s.blocks.find(x=>x.id===blockId); if(!b)return{...s,toast:"Block not found."}; const t=s.tasks[b.taskId]; if(!canEditDay(s,b.day)||!canEditDay(s,target.day))return{...s,toast:"That day is sealed."}; if(t?.kind==="review"&&isToday(s.weekStartIso,target.day))return{...s,toast:"Review blocks must be planned before the review day."}; return{...s,selectedDay:target.day,blocks:insertBlock(s.blocks.filter(x=>x.id!==blockId),{...b,day:target.day,timeBand:t?.kind==="review"?undefined:(target.timeBand??b.timeBand),timeStart:t?.kind==="review"?(target.timeStart??b.timeStart??REVIEW_DEFAULT_START):(target.timeStart??b.timeStart)},target)};});}
  ...
  function blockAction(id:string,action:"tier"|"done"|"missed"|"remove"|"pause"|"review"){const current=state.blocks.find(x=>x.id===id); const currentTask=current?state.tasks[current.taskId]:undefined; playSfx(!current||!currentTask?"error":action==="done"?(currentTask.kind==="reward"?"reward":currentTask.kind==="recovery"?"recover":"complete"):action==="missed"?"error":action==="pause"?"pause":action==="review"?"review":action==="remove"?"edit":"tick"); patch(s=>{const b=s.blocks.find(x=>x.id===id); if(!b)return{...s,toast:"Block not found."}; const t=s.tasks[b.taskId]; if(!t)return{...s,toast:"Action not found."}; if(action==="remove"||action==="tier"){if(!canEditDay(s,b.day))return{...s,toast:"Open Review before editing a sealed day."};}
    ...
    if(action==="done"){const tier=getTier(t,b.tier), already=b.status==="done"; if(already)return{...s,blocks:s.blocks.map(x=>x.id===id?{...x,status:"done"}:x)}; const xp=xpUnlockedForDay(s,b.day)?effectiveTierXp(t,tier):0; const player=applyPlayerGain(s.player,xp,tokenDeltaForTask(t),t.kind==="recovery"); const levelUp=player.level>rsLevelForXp(s.player.xp)?player.level:undefined; if(xp>0) emitRewardNearBlock(id,xp,levelUp,"done"); return{...s,blocks:s.blocks.map(x=>x.id===id?{...x,status:"done"}:x),player,toast:xp>0?undefined:"Done. Seal the day before completion to earn XP."};}
    if(action==="missed"){const existingRecovery=recoveryBlockForDay(s,b.day); const recoveryTimeStart=defaultFutureTimeStart(s.weekStartIso,b.day,Math.max(MIN_EVENT_MINUTES,getTier(s.tasks.recovery,1).minutes||30)); const recovery=makeBlock(s.tasks.recovery,b.day,nextOrder(s.blocks,b.day),undefined,undefined,recoveryTimeStart); const updated=s.blocks.map(x=>x.id===id?{...x,status:"missed" as Status}:existingRecovery&&x.id===existingRecovery.id&&x.status==="planned"?{...x,status:"recoveryDue" as Status,injected:x.injected}:x); return{...s,blocks:normalize([...updated,...(existingRecovery?[]:[{...recovery,tier:1 as const,status:"recoveryDue" as Status,injected:true}])]),player:{...s.player,streakState:"fractured"},toast:existingRecovery?"Miss noted. Existing recovery block reused.":"Recovery added."};}
    ...
    if(action==="review"){if(t.kind!=="review")return{...s,toast:"Not a Review block."}; return{...s,toast:"Review blocks open automatically on their scheduled day for 12 hours."};}
    return s;});}
  type D=Partial<Record<DayKey,DaySeal>>;
  function requestGate(){playSfx("review"); setState(s=>({...s,toast:"Add a Review Window to a future day. It opens automatically for 12 hours."}));}
  function openGate(){playSfx("review"); setState(s=>({...s,toast:"Review windows now open from planned Review blocks."}));}
  ...
  function renderBlock(b:Block, beamHit=false, timeMode=false, timeMeta?:{start:number;duration:number;end:number}){
    ...
    const reviewLive=t.kind==="review"&&reviewWindowIsOpen(state);
    ...
    const status=reviewLive?"Window open":timer?.label||(b.status==="planned"?(t.kind==="review"?"Scheduled":t.kind==="pause"?"Pause":"Ready"):b.status);
    const kindLabel=t.kind==="discipline"?"Discipline":t.kind==="reward"?"Dopamine":t.kind==="recovery"?"Recovery":t.kind==="review"?"Review":t.kind==="pause"?"Pause":"Action";
    const xp=effectiveTierXp(t,tier);
    const xpEligible=xpUnlockedForDay(state,b.day);
    const stats=[xp&&xpEligible?{spec:iconSprites.core.xp,label:`+${xp} XP`}:null,(!isDopamineTask(t)&&t.tokenEarn)?{spec:iconSprites.core.token,label:`+${t.tokenEarn}`}:null,t.tokenCost?{spec:iconSprites.core.token,label:`-${t.tokenCost}`}:null,tier.relief?{spec:iconSprites.core.streak,label:`-${tier.relief} tension`}:null].filter(Boolean) as {spec:SpriteSpec;label:string;locked?:boolean}[];
    ...
    return <article key={b.id} data-block-id={b.id} data-block-root data-drop-block={b.id} data-kind={t.kind} data-category={t.category} data-status={b.status} data-accent={t.accent} className={`block-card ${timeMode?"time-card":""} ${t.kind} ${t.accent} ${b.status} ${open?"expanded":""} ${reviewLive?"review-live":""} ${beamHit?"beam-hit":""}`} onClick={e=>{e.stopPropagation(); const el=e.currentTarget as HTMLElement; if(open) close(); else openPopover(el);}}>
      ...
      <div className="block-main">
        ...
        <div className="block-copy">
          ...
          {!!stats.length&&<div className="block-stat-row">{stats.map((item,i)=><span key={i} className={item.locked?"locked":undefined}><SpriteIcon spec={item.spec}/>{item.label}</span>)}</div>}
  ...
  function renderDay(day:DayKey){
    ...
    const pauseDay=isPauseDay(state,day);
    return <section key={day} data-drop-day={day} className={`day-column ${today?"today":""} ${past?"past":""} ${daySealed?"sealed":""} ${pauseDay?"pause-day":""} ${state.week.mode==="reviewOpen"?"review-open":""} ${state.selectedDay===day?"selected":""}`} title={dateTitle(state.weekStartIso,day)} onClick={()=>setState(s=>({...s,selectedDay:day}))}>
      {daySealed&&<div className="seal-trail" aria-hidden="true"><span/><span/><span/></div>}
      <header className="day-head"><div className="day-orb"><span>{day[0].toUpperCase()}</span></div><div><strong>{day.toUpperCase()}</strong><small>{dateShort(state.weekStartIso,day)}</small></div>{daySealed?<button className="sealed-button" onClick={e=>{e.stopPropagation(); unsealDay(day);}}>{state.week.mode==="reviewOpen"?"Unseal":"Sealed"}</button>:<button onClick={e=>{e.stopPropagation(); sealDay(day);}}>Seal</button>}</header>
  ...
  function renderTimeView(){
    ...
    return <div className="calendar-week">
      <div ref={calendarShellRef} className="calendar-grid-shell" style={{gridTemplateRows:`72px ${frameMinutes*CAL_PX_PER_MIN}px`,minHeight:72+frameMinutes*CAL_PX_PER_MIN}}>
        <div className="calendar-head-spacer" />
        {visibleDays.map(day=>{const sealed=isDaySealed(state,day); const past=isPast(state.weekStartIso,day); const today=isToday(state.weekStartIso,day); const pauseDay=isPauseDay(state,day); return <div key={day} className={`calendar-day-head ${today?"today":""} ${past?"past":""} ${sealed?"sealed":""} ${pauseDay?"pause-day":""} ${state.week.mode==="reviewOpen"?"review-open":""}`} onClick={()=>setState(s=>({...s,selectedDay:day}))} title={dateTitle(state.weekStartIso,day)}><header className="day-head calendar-day-head-card"><div className="day-orb"><span>{day[0].toUpperCase()}</span></div><div><strong>{day.toUpperCase()}</strong><small>{dateShort(state.weekStartIso,day)}</small></div>{sealed?<button className="sealed-button" onClick={e=>{e.stopPropagation(); unsealDay(day);}}>{state.week.mode==="reviewOpen"?"Unseal":"Sealed"}</button>:<button onClick={e=>{e.stopPropagation(); sealDay(day);}}>Seal</button>}</header></div>})}
  ...
  const reviewBanner=reviewNotice(state);
  const showReviewBanner=!!reviewBanner&&hiddenNoticeKey!==reviewBanner.key&&!readNoticeDismissed(reviewBanner.key);
  return <main className="app-shell" style={appAssetVars}>
    ...
    {showReviewBanner&&reviewBanner&&<section className={`system-window-banner ${reviewBanner.active?"active":"scheduled"}`}><strong>{reviewBanner.title}</strong><span>{reviewBanner.detail}</span><button className="system-window-close" type="button" onClick={()=>{dismissNotice(reviewBanner.key); setHiddenNoticeKey(reviewBanner.key);}}>×</button></section>}
    <section className={`workspace ${state.activeView==="plan"?"plan-workspace":"single-workspace"}`}>
      {state.activeView==="plan"&&<>
        <aside className="action-bank glass-panel">
          ...
          <div className="chip-row">{(["All",...CATEGORY_OPTIONS] as const).map(c=><button key={c} className={chip===c?"active":""} onClick={()=>setChip(c)}>{c}</button>)}</div>
        ...
        <section className="week-panel glass-panel">
          <div className="week-head"><div><h1>Week</h1><p>{state.week.mode==="reviewOpen"?"Review window open for 12 hours. Edit freely, then reseal.":"Drag actions into days. Seal one day or the full week when it feels stable."}</p></div><div className="week-controls">
