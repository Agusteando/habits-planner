...
function reviewNotice(s:State,now=new Date()){if(reviewWindowIsOpen(s,now)){const end=currentReviewEnd(s)!; return{active:true,title:"Review window open",detail:`All sealed days are editable until ${end.toLocaleTimeString([], {hour:"numeric",minute:"2-digit"})}. Reseal when done.`};} const next=nextReviewWindow(s,now); if(next)return{active:false,title:"Next review window",detail:`${next.start.toLocaleString([], {weekday:"long",hour:"numeric",minute:"2-digit"})}–${next.end.toLocaleTimeString([], {hour:"numeric",minute:"2-digit"})}`}; return null;}
...
function App(){
  ...
  function renderBlock(b:Block, beamHit=false, timeMode=false, timeMeta?:{start:number;duration:number;end:number}){
    ...
    const stats=[xp?{spec:iconSprites.core.xp,label:xpEligible?`+${xp} XP`:`Seal first · ${xp} XP locked`,locked:!xpEligible}:null,(!isDopamineTask(t)&&t.tokenEarn)?{spec:iconSprites.core.token,label:`+${t.tokenEarn}`}:null,t.tokenCost?{spec:iconSprites.core.token,label:`-${t.tokenCost}`}:null,tier.relief?{spec:iconSprites.core.streak,label:`-${tier.relief} tension`}:null].filter(Boolean) as {spec:SpriteSpec;label:string;locked?:boolean}[];
  ...
  return <main className="app-shell" style={appAssetVars}>
    <header className="topbar glass-panel">
      ...
      <div className="top-status">
        ...
        <span className={`top-chip mode ${state.week.mode}`}>{state.week.mode==="reviewOpen"?`Review ${countdown(state.week.reviewOpenUntil)}`:state.week.mode==="sealed"?"Sealed":"Draft"}</span>
      </div>
    ...
    {reviewBanner&&<section className={`system-window-banner ${reviewBanner.active?"active":"scheduled"}`}><strong>{reviewBanner.title}</strong><span>{reviewBanner.detail}</span></section>}
