...
function hasAnySealed(s:State){return s.week.mode==="sealed"||s.week.mode==="reviewPending"||Object.keys(s.week.daySeals||{}).length>0;}
function firstSealedDay(s:State){return (DAYS.find(d=>!!s.week.daySeals[d])||s.selectedDay) as DayKey;}
...
function currentLineInfo(weekStartIso:string,day:DayKey,filter:TimeFilter){if(!isToday(weekStartIso,day))return null; const bands=visibleBands(filter); if(!bands.length)return null; const start=bandRange(bands[0].id).start; const end=bandRange(bands[bands.length-1].id).end; const now=new Date(); const mins=now.getHours()*60+now.getMinutes(); if(mins<start||mins>end)return null; const pct=(mins-start)/(end-start)*100; const currentBand=(bands.find(b=>mins>=bandRange(b.id).start&&mins<bandRange(b.id).end)?.id)||bands[bands.length-1].id; const label=now.toLocaleTimeString([], {hour:"numeric", minute:"2-digit"}); return {pct,currentBand,label,minutes:mins};}
...
function isSystemTask(t?:Task){return !!t&&(t.kind==="review"||t.kind==="pause"||t.kind==="recovery");}
...
function App(){
  ...
  useEffect(()=>{
    ...
    if(hitId){
      ...
      width=hitRect?Math.max(24,(hitRect.left-shellRect.left+20)-origin):Math.max(24,(laneRect.right-shellRect.left)-origin);
    }else{
      width=Math.max(24,(laneRect.right-shellRect.left)-origin);
    }
    setBeamGeom({top:(line.minutes-frame.start)*CAL_PX_PER_MIN,left:origin,width,hasTarget:!!hitId});
  ...
  function sealDay(day:DayKey){patch(s=>{if(s.week.daySeals[day])return{...s,toast:"Day already sealed."}; const sealedBlocks=s.blocks.map(b=>b.day===day?{...b,sealed:true}:b); const next={...s,blocks:sealedBlocks,week:{...s.week,daySeals:{...s.week.daySeals,[day]:{sealedAt:new Date().toISOString()}}}}; return{...next,blocks:ensureReviewBlock(next,day),toast:"Day sealed. Review block added."};});}
  function unsealDay(day:DayKey){patch(s=>{if(!s.week.daySeals[day])return{...s,toast:"That day is not sealed."}; if(s.week.mode!=="reviewOpen")return{...s,toast:"Open a Review block before unsealing."}; const seals={...s.week.daySeals}; delete seals[day]; return{...s,week:{...s.week,daySeals:seals},blocks:s.blocks.map(b=>b.day===day?{...b,sealed:false}:b),toast:"Day unsealed."};});}
  ...
  function fullWipe(){if(!window.confirm("Full reset wipes level, XP, tokens, streak, completion state, and seals. Continue?"))return; setState(s=>({...s,blocks:s.blocks.map(b=>({...b,status:"planned",sealed:false,reviewOpenedAt:undefined,pauseAppliedAt:undefined})),player:{level:1,xp:0,tokens:0,streakDays:0,streakState:"healthy",resetCount:s.player.resetCount+1},week:{mode:"draft",daySeals:{}},theme:s.theme,toast:"Full reset complete. Progress lost; plan unsealed."}));}
  ...
  function renderBlock(b:Block, beamHit=false, timeMode=false, timeMeta?:{start:number;duration:number;end:number}){
    ...
    const status=b.status==="planned"?(t.kind==="review"?countdown(b.reviewUnlockAt):t.kind==="pause"?"Pause":"Ready"):b.status;
  ...
  function renderDay(day:DayKey){
    const blocks=dayBlocks(state.blocks,day);
    const daySealed=!!state.week.daySeals[day];
...
function Vault({state,exportPlan,fullWipe}:{state:State;exportPlan:()=>void;fullWipe:()=>void}){return <section className="vault-panel glass-panel"><div className="vault-hero"><span>✦</span><h1>Vault</h1><p>{state.player.tokens} tokens · {state.player.xp} XP · reset count {state.player.resetCount}</p></div><div className="reset-card"><div><strong>Full reset</strong><p>Lose level, XP, tokens, streak, and completion progress. The plan is unsealed, but your planned blocks and Action Bank stay.</p></div><button className="danger-reset" onClick={fullWipe}>Reset progress</button></div><div className="vault-actions"><button onClick={exportPlan}>Export plan</button></div><div className="vault-list"><div>Full reset is the costly escape hatch. Reset count is preserved.</div><div>Review Window and Pause Day are protected system blocks.</div><div>System blocks cannot be deleted from the board or Action Bank.</div><div>The normal way to edit sealed plans is through a Review block.</div></div></section>}
