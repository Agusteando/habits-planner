...
const tasksSeed: Record<string, Task> = {
  ...
  review: actionTask("review","Review Window","review","System","silver",0,0,["15 min edit window","30 min review","45 min reset"],[15,30,45],[0,0,0],[0,0,0],[8,10,12]),
...
function makeCustom(title:string,kind:Kind):Task{const clean=title.trim(); const idText=clean.toLowerCase().replace(/[^a-z0-9]+/g,"_").replace(/^_|_$/g,"").slice(0,38)||"custom"; const c={discipline:["Body","cyan",0,1,[4,16,34],[10,30,60],["Minimum version","Standard version","Deep version"]],reward:["Dopamine","violet",1,0,[0,0,0],[0,0,0],["5 min cap","10 min cap","15 min hard cap"]],recovery:["System","amber",0,0,[0,2,4],[8,16,24],["Tiny reset","Reset","Full reset"]],review:["System","silver",0,0,[0,0,0],[0,0,0],["15 min edit window","30 min review","45 min reset"]],pause:["System","silver",0,0,[0,0,0],[0,0,0],["Pause remaining day","Protect evening","Full stop"]]}[kind] as [Category,Accent,number,number,number[],number[],string[]]; const relief=kind==="reward"?[5,9,12]:kind==="recovery"?[12,20,30]:kind==="review"?[8,10,12]:kind==="pause"?[20,30,40]:[0,0,0]; return normalizeTask({id:`custom_${idText}_${Date.now().toString(36)}`,title:clean,icon:"sprite",kind,category:c[0],accent:c[1],tokenCost:c[2],tokenEarn:c[3],tiers:[1,2,3].map((l,i)=>({level:l as 1|2|3,label:c[6][i],minutes:kind==="pause"?0:[5,20,45][i],xp:c[5][i],tension:c[4][i],relief:relief[i]}))});}
...
function App(){
  ...
  function addTemplate(taskId:string,target:DropTarget){playSfx(!canEditDay(state,target.day)||!state.tasks[taskId]?"error":"drop"); patch(s=>{if(!canEditDay(s,target.day))return{...s,toast:"That day is sealed."}; const t=s.tasks[taskId]; if(!t)return{...s,toast:"Action not found."}; const timeStart=defaultTimeStartForNewBlock(s.weekStartIso,target.day,t,target); return{...s,selectedDay:target.day,blocks:insertBlock(s.blocks,makeBlock(t,target.day,nextOrder(s.blocks,target.day),undefined,target.timeBand,timeStart),target)};});}
  ...
  function requestGate(){playSfx(hasAnySealed(state)?"review":"error"); patch(s=>{if(!hasAnySealed(s))return{...s,toast:"Seal a day or week first."}; const unlock=new Date(Date.now()+REVIEW_DELAY).toISOString(); const next={...s,week:{...s.week,mode:"reviewPending" as Mode,emergencyReviewRequestedAt:new Date().toISOString(),emergencyReviewUnlockAt:unlock}}; return{...next,toast:"Review Gate requested. Add or open a Review block manually."};});}
