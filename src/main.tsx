...
function App(){
  ...
  async function importBank(file?:File){if(!file)return; try{const bank=validateBank(JSON.parse(await file.text())); const importedIds=Object.keys(bank.tasks); setState(s=>({...s,tasks:{...s.tasks,...bank.tasks},deletedTaskIds:s.deletedTaskIds.filter(id=>!importedIds.includes(id)),toast:`Action bank imported (${importedIds.length}).`}));}catch(e){setState(s=>({...s,toast:e instanceof Error?e.message:"Action bank import failed."}));}}
